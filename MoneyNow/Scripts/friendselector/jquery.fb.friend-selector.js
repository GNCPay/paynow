(function($) {
  ...
  $.fn.fbFriendSelector = function(friends, selectedFriends) {
    ...
    $fs
      ...
      .append('<a href="#" class="form_button">Gửi</a>');

    var $fsc = $('#friend_selector_controls');

    $fsc
      .append('<input id="friend_selector_search" name="friend_selector_search" type="text" placeholder="Nhập tên bạn bè" />')
      .append('<a href="#" class="show_selected">Đã chọn (0)</a>')
      .append('<a href="#" class="show_all highlight">Tất cả</a>')
    ...
    $.each(friends, function() {
      ...
      $fsf.append('<li data-friend-id="'+this.id+'" data-friend-name="'+this.name+'" class="'+selected+'"><span class="frame"><img src="'+this.image+'" width="50" height="50" /><span class="check"></span></span><span class="name">'+this.name+'</span></li>');
    });
    updateSelectedCount();
    ...
    function updateSelectedCount() {
      $showSelected.text('Đã chọn ('+$fsf.find('li.selected').length+')');
    }
